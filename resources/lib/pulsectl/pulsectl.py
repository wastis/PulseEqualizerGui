...
class Pulse(object):
	...
	def card_port_set_latency(self, card_name, port_name, latency):
		card_name, port_name = map(c.force_bytes, [card_name, port_name])
		with self._pulse_op_cb() as cb:
			try: c.pa.context_set_port_latency_offset(self._ctx, card_name, port_name, latency, cb, None)
			except c.pa.CallError as err: raise PulseOperationInvalid(err.args[-1])
